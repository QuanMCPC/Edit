...
require("fs").readFile(require("path").join(__dirname, "settings.json"), { encoding: "utf-8" }, (_err, data) => {
    ...
    document.getElementById("apply").addEventListener("click", () => {
        ...
        if (settings.minWidth > document.getElementById("width").value) { settings.width = settings.minWidth; document.getElementById("width").value = settings.minWidth } else { settings.width = Number(document.getElementById("width").value) }
        if (settings.minHeight > document.getElementById("height").value) { settings.height = settings.minHeight; document.getElementById("height").value = settings.minHeight } else { settings.height = Number(document.getElementById("height").value) }        settings.noUpdateOnStartup = document.getElementById("noUpdateOnStartup").checked
        ...
        settings.allowBetaUpdate = document.getElementById("allowBetaUpdate").checked
        ...
        require("fs").writeFile(require("path").join(__dirname, "settings.json"), JSON.stringify(settings), () => {
            ...
            var t = setTimeout(() => { document.getElementById("saved").innerHTML = "" }, 15000)
    ...
    document.getElementById("reset").addEventListener("click", () => {
        require("fs").writeFile(require("path").join(__dirname, "settings.json"), `{"backgroundTextColor":"#000000","textColor":"#ffffff","width":800,"height":600,"minWidth":360,"minHeight":240,"noUpdateOnStartup":false,"unResizable":false,"characterLimit":"0","autoRecovery":false,"autoRecoveryInterval":0,"showInfoBar":false,"allowBetaUpdate":false}`, () => {
